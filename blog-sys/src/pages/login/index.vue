...
<script>
  ...
  class Vue {
    ...
    methods = {
      init() {
        if (this.$VuserInfo.menu) {
          this.setPath();
        }
      },
      login() {
        if (this.form.account && this.form.password) {
          this.$Alogin({body: this.form}).then(() => {
            storage[this.form.radio ? 'setLocal' : 'setSession']({key: 'userInfo', value: this.$VuserInfo});
            if (this.$VuserInfo.menu) {
              this.setPath();
            }
          });
      ...
      setPath() {
        this.$router.push(this.$VuserInfo.menu[0].childList[0].href);
      }
    };
    created = function () {
      this.init();
