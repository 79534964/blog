<template>
    <div class="app_lock" v-show="lockFlag">
...
<script>
    ...
    export default {
        ...
        methods: {
            init() {
                if (this.form.password) {
                    this.$store.dispatch('common/act/LOCKFLAG', true);
                }
            },
            getPassword() {
            ...
            change() {
                window.setTimeout(() => {
                    ...
                    this.backgroundClass = this.lockFlag ? 'background block' : 'background';
                    this.lockFlag && this.$mapping({entity: this.form});
                    this.getPassword();
                }, 100);
            },
            clear() {
            ...
            close() {
                ...
                window.setTimeout(() => {
                    this.$store.dispatch('common/act/LOCKFLAG', false);
                }, 300);
            },
            setInputClass() {
        ...
        computed: {
            lockFlag() {
                return this.$store.getters['common/get/LOCKFLAG'];
            },
            userInfo() {
